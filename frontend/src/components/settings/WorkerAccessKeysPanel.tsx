...
export default function WorkerAccessKeysPanel() {
  ...
  const [name, setName] = useState("puddingteams-local");
  ...
  return <SettingsCard title="Worker 接入" icon={KeyRound} color="#002fa7">
    <div className="space-y-5">
      ...
      <div className="flex justify-end">
        <button type="button" onClick={refresh} disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50"><RefreshCw className="h-3.5 w-3.5" />加载列表</button>
      </div>
      <div className="grid gap-3 md:grid-cols-[1fr_180px_auto]">
        <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Key 名称" className="form-input" />
