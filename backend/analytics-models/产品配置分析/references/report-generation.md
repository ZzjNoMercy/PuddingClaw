...
- 图表 DOM id 集合必须等于 reference 的 25 张图表集合。
- 每个任务必须声明输出路径与空数据策略。
...
- 封面只显示报告标题与报告日期。
- 8 个章节和 25 个图表容器存在。
- 有数据的 25 张图均生成 canvas/SVG；无数据图显示明确空状态。
...
7. **数据口径**：数据源、截止日期、范围、颗粒度、分子分母、排除项、缺失与异常。
8. **核心配置率查询**：置于报告最后；按配置项、款型/车系口径及价格段/轴距段/级别查询历年搭载规模与配置率。
...
| `premiumHudChart` | 折线图 | 年份/价格带、高价位 HUD 覆盖率 |
...
- [ ] 封面只包含报告标题和报告日期。
- [ ] 8 个必需章节均存在且顺序正确，“核心配置率查询”位于最后。
- [ ] 25 个图表容器均存在；有数据时全部生成 canvas 或 SVG。
