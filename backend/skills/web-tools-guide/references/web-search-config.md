...
3. **在操作前请求确认**（原样输出以下内容）：
   > 接下来我将进行以下操作：
   > 1. 将 API Key 配置到当前 Agent 环境（环境变量或配置文件）
   > 2. 应用配置后重新尝试搜索
   >
   > ⚠️ **重要提示**：
...
1. **配置 API Key 到当前 Agent 环境**：
   ```bash
   # Tavily（tvly-* 前缀）
   export TAVILY_API_KEY="<user-provided-key>"

   # Kimi（sk-* 前缀）
   export KIMI_API_KEY="<user-provided-key>"
   ```

   如果 Agent 使用配置文件，请按实际配置格式将 Key 写入对应位置。
...
1. **应用配置**：
   根据当前 Agent 的部署方式刷新配置：
   - 如果是环境变量，重新加载环境变量后重启 Agent 进程
   - 如果是配置文件，保存后重启 Agent 服务
   - 如果配置已热加载，可直接继续下一步
