...
4. For a new dimension, let `enqueue_semantic_dimension_build` create `semantic-assets/dimensions/<dimension_id>/dimension.md` and its `references/` directory from the semantic-dimension template. Do not use a generic file-writing tool. For an existing dimension, the enqueue path reuses its package. A newly inspected source can either append a registered source or register a new source; neither action creates a new canonical field unless the user explicitly requests a schema extension.
...
- For legacy `vehicle_series_full`, `vehicle_model_base.brand + serial_name` remains the registered fast-path canonical universe. This is an adapter-specific rule, not a platform-wide default.
