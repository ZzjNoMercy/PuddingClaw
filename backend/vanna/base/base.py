...
class VannaBase(ABC):
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, **kwargs) -> str:
        ...
        if 'intermediate_sql' in llm_response:
            # 修改：拦截 intermediate_sql（当存在 entity_list 时）
            if entity_list and len(entity_list) > 0:
                # 如果有实体映射，强制 LLM 直接使用，不允许 intermediate_sql
                force_prompt = initial_prompt or f"You are a {self.dialect} expert. " + \
                    "Please help to generate a SQL query to answer the question. Your response should ONLY be based on the given context and follow the response guidelines and format instructions. "
                force_prompt += (
                    "\n\n===重要提醒\n"
                    "你刚才尝试生成中间SQL查询，这是不必要的。\n"
                    "上面的实体映射表中已经包含了你需要的准确值。\n"
                    "请立即使用这些标准值生成最终的SQL查询。\n"
                    "禁止在响应中包含'intermediate_sql'。\n"
                )
                
                # 重新构造 prompt
                message_log = [self.system_message(force_prompt)]
                
                # 添加 DDL
                if ddl_list:
                    message_log.append(self.user_message(f"表结构:\n" + "\n".join(ddl_list)))
                ...
                self.log(title="LLM Response (Retry)", message=llm_response)
            elif not allow_llm_to_see_data:
                return "The LLM is not allowed to see the data in your database. Your question requires database introspection to generate the necessary SQL. Please set allow_llm_to_see_data=True to enable this."
            elif allow_llm_to_see_data:
                ...
                try:
                    ...
                    prompt = self.get_sql_prompt(
                        ...
                        entity_list=entity_list,  # 保持 entity_list
                        **kwargs,
                    )
    ...
    def get_related_entities(self, question: str, **kwargs) -> list:
        """
        This method is used to get related entity mappings to a question.
        Entity mappings help map user input aliases to canonical database values.

        Args:
            question (str): The question to get related entity mappings for.

        Returns:
            list: A list of entity mappings with keys: entity_type, canonical_name, aliases, table_column, score
        """
        pass
    ...
    def add_entity(self, canonical_name: str, entity_type: str, aliases: list = None, table_column: str = None, **kwargs) -> str:
        """
        This method is used to add an entity mapping to the retrieval layer.
        Entity mappings help map user input aliases to canonical database values.

        Args:
            canonical_name (str): The canonical database value.
            entity_type (str): User-defined entity label (e.g., 'region', 'product_name', 'customer_segment').
            aliases (list): A list of alternative names that users might use.
            table_column (str): The database table.column where this entity is used (e.g., 'orders.region').

        Returns:
            str: The ID of the entity that was added.
        """
        pass
    ...
    def get_all_entities(self, entity_type: str = None, **kwargs) -> list:
        """
        This method is used to get all entity mappings from the retrieval layer.

        Args:
            entity_type (str): Optional filter by user-defined entity type (e.g., 'region', 'product_name').

        Returns:
            list: A list of entity mappings.
        """
        pass
    ...
    def remove_entity(self, id: str, **kwargs) -> bool:
        """
        This method is used to remove an entity mapping from the retrieval layer.

        Args:
            id (str): The ID of the entity to remove.
    ...
    def add_entities_to_prompt(self, initial_prompt: str, entity_list: list, **kwargs) -> str:
        """
        将用户确认过的实体映射添加到 Prompt。

        Entity 是通用的业务标准值映射，不预设任何行业字段。它可以表示地区、产品、
        客户、渠道、状态或任意用户定义的标签；具体含义由 entity_type 和
        table_column 决定。
        """
        if entity_list and len(entity_list) > 0:
            initial_prompt += "\n===Entity Mappings（用户确认过的标准值）\n"
            initial_prompt += (
                "The following entity mappings are curated business values. "
                "When the user's question mentions an alias or fuzzy name, use the corresponding "
                "canonical_name in WHERE clauses. Do not invent replacement values.\n\n"
            )
            ...
            initial_prompt += "1. Use canonical_name exactly as shown for equality or IN filters.\n"
            initial_prompt += "2. Use table_column when it is provided; do not move the value to an unrelated column.\n"
            initial_prompt += "3. Aliases are only for matching user wording; aliases should not be used as SQL literal values unless they are also canonical_name.\n"
            initial_prompt += "4. If multiple entity mappings match the question, choose the smallest set that answers the question; use IN (...) only when the user asks for a group or comparison.\n"
            initial_prompt += "5. If the mappings are insufficient, fall back to the normal intermediate_sql mechanism to inspect distinct values.\n"
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        Args:
            ...
            entity_list (list): A list of entity mappings (optional).
        ...
        initial_prompt += (
            ...
            "7. If entity mappings are provided above, use the canonical_name values in WHERE clauses instead of user input aliases, and prefer the mapped table_column when available. \n"
        )
