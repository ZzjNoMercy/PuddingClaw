...
After one successful `llamaindex_knowledge_query`, treat its structured sources as authoritative for that question. Do not use `ls`, `glob`, or `grep` as a fallback. If more source text is necessary, call `read_file` only on the exact `linked_markdown_virtual_path` returned by the retrieval.
