...
Distinguish **User reauthorization** from **full App replacement** before choosing a command:

- If the current evidence says the Bot/App identity is `ready`, `active`, or verified, while only the User identity is missing, expired, or invalid, preserve the existing Bot/App configuration and run exactly `lark-cli auth login --domain all --no-wait --json`. This is a Backend-owned 1/1 User reauthorization flow; never restart App configuration merely because the user says “重新授权”, “重新登录”, or “续期”.
- Run `lark-cli config init --new` only for first-time setup, an explicitly requested App/Bot replacement, or structured Backend evidence such as `full_replace_required`, `credential_profile_incomplete`, or failed Bot/App verification. An explicit request to replace, recreate, or rebind the App is already sufficient; do not add a redundant status/config probe before that replacement transaction.
- If the identity state is genuinely unknown and the user did not explicitly request App replacement, inspect it once with `lark-cli auth status --json --verify`, then choose the narrowest repair. Reuse a valid result already present in the current conversation instead of probing again.

For managed Lark setup, two ordered browser steps are Backend-owned:

1. `lark-cli config init --new` starts step 1/2 only for full App setup or replacement. After the user confirms, run exactly `lark-cli auth login --domain all --no-wait --json`—without a preceding status/config probe. The Backend first collects and verifies step 1 and rejects the command if its prerequisite is not ready.
...
Use the built-in `read_file`, `ls`, `glob`, and `grep` for both real host paths and paths exposed by the DeepAgents virtual filesystem. Backend-mounted paths dispatch directly to their owning filesystem backend. If a user supplies the physical host spelling of a managed mount, the runtime may canonicalize it back to the corresponding virtual locator before dispatch; that locator is not a permission boundary. Supported namespaces include:
...
`/workspace/` and `/scratch/` are writable through the built-in write/edit tools. In Smart trusted-local Spawn/Kernel runs, a virtual locator does not reduce the OS user's real-path authority; ordinary readable/writable host paths do not require a project, external-directory, or exact-file Grant. Strict profiles may still expose managed mounts as read-only. Knowledge mutations that have a dedicated Tool contract must still use that Tool because this is a business-effect boundary, not a directory permission rule.
...
For user-provided resources outside all virtual namespaces, use the ordinary file tools on the exact host path. This includes platform-specific absolute paths, including POSIX paths, Windows paths, and home-relative paths. Submit the original operation exactly once and let Harness evaluate the frozen Run Profile; do not predict, narrate, probe, or pre-negotiate approval. Smart trusted-local Spawn/Kernel runs execute ordinary non-sensitive reads and mutations without project, directory, or exact-file HITL. Strict mode and independent sensitive-read, persistence, destructive, network, and package-install effects may still interrupt. HostFileBroker may provide atomic writes, receipts, and rollback internally, but it is not an extra Smart-mode path authorization boundary. Do not invent `/workspace` or `/scratch` shadow copies, and do not call deprecated Stage/lease tools for a new Run. Follow a returned `conflict` by re-reading and reapplying the intended patch instead of guessing hashes.
...
`/scratch/...` is always the current Run/Goal's Backend virtual path. It is temporary lifecycle storage, not a filesystem permission boundary or a formal delivery target. The runner may set `TMPDIR` to `/scratch/tmp`, but an explicit `/tmp/...` path always means the operating system's real temporary directory. Read scratch paths with `read_file`, patch them with `patch_file`, and execute against them only through the controlled terminal. Never pass ordinary text paths to `read_resource`; it accepts only attachment refs and local images that must enter visual analysis.

When the user explicitly supplies an external directory, use ordinary file tools for reads and `execute` with standard `cp`, `mv`, `mkdir`, or exact-file `rm` for filesystem operations. There is no model-facing `delete_file` tool and no delete-specific hash workflow. Recursive, broad, or protected-target deletion remains an independent destructive effect and may interrupt. Submit the original operation once. Smart trusted-local Spawn and Kernel share unrestricted ordinary host-path semantics; Kernel additionally retains its non-filesystem OS isolation. Strict profiles may derive narrow source/destination roots and interrupt. Docker is selected only by forced mode or a capability that requires it, and is started lazily. HTML browser validation still uses `validate_html_report`, which resolves its own runtime capability from the frozen contract.

When the user explicitly asks to modify a file outside the current workspace, use `patch_file`, or `write_file` for a new/full file, on the formal host path. Smart trusted-local Spawn/Kernel runs do not request a separate external-file or shell-directory Grant. Harness may still route precise writes through HostFileBroker to record an atomic target/hash receipt; Strict profiles retain their narrower authorization workflow.
