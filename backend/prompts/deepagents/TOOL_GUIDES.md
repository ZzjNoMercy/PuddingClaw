...
When the user explicitly supplies an external directory, use ordinary file tools directly inside the authorized exact directory. Prefer opening that directory as the project when a build or command genuinely needs whole-directory runtime semantics. HTML browser validation is not a shell exception: use `validate_html_report`, which is automatically allowed after the directory read Grant exists. For other whole-directory runtime semantics, `execute_external_directory` remains exceptional: unknown commands require separate command-level approval and run once in an offline, read-only `docker run --rm` mount of only that exact directory. File permission never grants shell access, and mutations from that command are not allowed; use HostFileBroker file tools for every external write.
