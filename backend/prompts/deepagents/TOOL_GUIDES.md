...
For user-provided resources outside all virtual namespaces, call `read_resource`. This includes platform-specific absolute paths, including POSIX paths, Windows paths, and home-relative paths. Exact-file read authorization permits `grep` on that exact file only; it never exposes siblings. An approved exact-directory search is automatically routed through an isolated read-only snapshot, so call the original `ls`, `glob`, or `grep` on the authorized host directory and let Harness rewrite it—do not guess scratch paths. If sibling discovery is genuinely required but not yet authorized, the search call opens the exact-directory read flow. For modification, call `stage_external_artifact` with the exact original path, edit the returned `/scratch/external/<lease_id>/...` file, and write validation scripts only under the returned `/scratch/validation/<lease_id>` directory. Inspect the edited draft hash, validate those exact bytes, then call `commit_external_artifact` with `expected_draft_sha256` and the successful `validation_receipt_ids`. Omit `expected_source_sha256` unless conflict diagnosis requires it; when supplied it is the immutable source hash captured at staging, not the edited staged-file hash. Never edit the host path directly, copy a `/workspace` shadow over the lease, or treat a scratch/workspace copy as delivery. Harness permits only one writable lease covering a formal target at a time.
...
When the user explicitly supplies an external directory for modification or execution, call `stage_external_directory` to create the authoritative writable snapshot under `/scratch/external-directories/<lease_id>/`. Put temporary validators under the separate `validation_scratch` path. For whole-directory modification, execution, or debugging, first recommend opening the directory as the project. If the user continues, operate only on the lease; a target already covered by an exact-file lease cannot also be staged through a directory lease. When write-back is required, call `prepare_external_directory_commit` with every intentional new deliverable in `declared_delivery_files`; undeclared additions fail closed so validation scripts cannot leak into delivery. Validate every added or modified code-like file from this exact staged tree, then call `commit_external_directory` with all successful `validation_receipt_ids`. Recursive write approval may be Run- or Session-scoped according to the user's choice and remains exact-directory scoped.

When the user explicitly asks to modify a file outside the current workspace, never call `edit_file`, `write_file`, or `patch_file` on the host path. Use the external-artifact staging and commit workflow above. External file approval never grants directory-wide access.
