...
class BrainSchemaService:
    ...
    def _render_agents(
        ...
        return (
            ...
            "- 对微信公众号、Newsletter、博客、播客频道、视频频道等具有稳定名称且会重复收录内容的发布渠道，"
            "应建立或复用一个 `source` 页面；先从 Index 解析已有 slug，再使用 `sourced_from` 将每个 `media` 页面链接到该来源，"
            "禁止为同一平台账号重复建页。首次建页时只记录 Raw 直接支持的账号名、平台及已收录内容，不得补写外部事实。\n"
            "- `source` 页的“已收录内容”必须使用完整路径 wikilink 指向对应 `media` 页面："
            "`[[media/<slug>|文章标题]]`。禁止使用“详见 media 页面”等纯文本代替链接。"
            "对于同一收录关系，`media` 必须以 `sourced_from` 指向 `source`，`source` 也必须在“已收录内容”中反向链接 `media`，"
            "确保双向可导航；不得重复收录同一页面。\n"
            "- 账号名或频道名本身不等于 `person`。只有 Raw 明确支持具体自然人身份时才建立 `person` 页面和 `authored` 关系；"
            "仅有署名时保留原始署名，不创建空洞人物页。\n"
            "- Raw 自带的 `id`、`type`、`subtype`、`related` 等 frontmatter 是来源元数据和分类线索，"
            "不是目标 Wiki 的分类结论；必须根据正文主题和活动 Schema 重新判断 page type 与关系。\n"
            "- 编写页面前先在内部完成“长期实体—稳定主题—关系”清单。"
            "对 Raw 明确出现且证据充分的具体框架、系统、模型、语言和工程实践分别建页；"
            "一份 raw 可以编译出多个 Wiki 页面，一个页面只描述一个长期稳定主题，不按 raw 文件机械地一对一建页。\n"
            "- 具体框架与作用于该框架的工程实践应分别建为 `software_framework` 和 `engineering_practice`；"
            "若框架只有名称而缺少足以形成稳定页面的事实，则保留为正文文本并报告知识缺口，不得用模型常识补全。\n"
            "- 本次选中的 Raw 正文是事实依据。`source_refs`、文件路径、URL 和其他引用字段只表示来源线索；"
            "除非对应内容也作为本次 Raw 被授权读取，否则不得递归读取或据此引入事实。\n"
            "- 现有 `index.md` 只用于发现和解析已有页面 slug，不是事实证据；"
            "不得根据 Index 摘要推断归属、依赖、实现方或其他关系。\n"
            "- 每项事实、实体归属和关系都必须由本次 Raw 直接支持。"
            "严格保留专有名词及主客体，不得把不同框架、系统、公司或项目互换；模型既有知识不能作为补充证据。\n"
            "- 只在 Raw 明确支持关系时创建 wikilink。不得为了避免孤立页面或满足“互链”而添加关系；"
            "暂时没有可信关系的页面可以保持孤立。若关系两端均有充分证据且页面尚不存在，应在同一次 publish 中创建缺失页面。\n"
            ...
            "## Ingest（摄取）\n\n"
            "开始前读取 `AGENTS.md`、当前 Schema Bundle、本次选中的 raw 文件和 Wiki Index。"
            "先完成实体、主题、页面类型和有证据关系的规划，再生成页面；"
            "发布前逐项核对页面中的事实归属和关系是否可由本次 Raw 直接支持。"
