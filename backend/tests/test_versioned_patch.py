...
from tools.filesystem.leases import safe_staged_filename as _safe_staged_filename
...
def test_versioned_patch_rebases_once_and_reports_structured_conflicts(tmp_path):
    ...
    path.write_text("A\nB\n", encoding="utf-8")
    middleware = VersionedPatchMiddleware(FilesystemBackend(root_dir=tmp_path, virtual_mode=True))
...
def test_replace_file_tool_replaces_workspace_file_without_broker_permission(tmp_path):
    ...
    replace_tool = next(tool for tool in middleware.tools if tool.name == "replace_file")

    result = replace_tool.func(
        file_path="/workspace/report.html",
        content="<html>after</html>\n",
        expected_sha256=_digest("<html>before</html>\n"),
        runtime=_runtime("replace-workspace"),
    )

    assert result.status == "success"
    payload = json.loads(result.content)
    assert payload["status"] == "completed"
    assert payload["authority_kind"] == "workspace"
    assert "permission_required" not in result.content
    assert target.read_text(encoding="utf-8") == "<html>after</html>\n"


def test_inspect_file_version_can_return_metadata_without_reinjecting_content(
    tmp_path,
):
    path = tmp_path / "large-report.js"
    content = "const payload = " + repr("x" * 40_000) + ";\n"
    path.write_text(content, encoding="utf-8")
    middleware = VersionedPatchMiddleware(FilesystemBackend(root_dir=tmp_path, virtual_mode=True))
    inspect_tool = next(tool for tool in middleware.tools if tool.name == "inspect_file_version")
    schema = inspect_tool.args_schema.model_json_schema()

    metadata_only = inspect_tool.func(
        file_path="/large-report.js",
        include_content=False,
        runtime=_runtime("inspect-metadata"),
    )
    compatible_default = inspect_tool.func(
        file_path="/large-report.js",
        runtime=_runtime("inspect-default"),
    )

    assert metadata_only.status == "success"
    assert schema["properties"]["include_content"]["default"] is True
    assert "only the latest sha256 is needed" in schema["properties"]["include_content"]["description"]
    assert f"version: {_digest(content)}" in metadata_only.content
    assert f"size_chars: {len(content)}" in metadata_only.content
    assert "\ncontent:\n" not in metadata_only.content
    assert content not in metadata_only.content
    assert compatible_default.status == "success"
    assert f"\ncontent:\n{content}" in compatible_default.content
    ...
    def receipt(
        ...
        *,
        ...
        return {
            ...
            "blocking": True,
            "failure_class": ("invocation_failure" if status == "failed" else None),
...
def test_validate_html_report_emits_hash_bound_browser_receipt(tmp_path) -> None:
    ...
    tool = next(item for item in VersionedPatchMiddleware(backend).tools if item.name == "validate_html_report")
    ...
    assert execution.commands == [
        (
            "node /opt/puddingclaw/bin/validate-html-report-e2e.mjs /workspace/report.html",
            120,
        )
    ]
    ...
    receipt = next(
        ref for activation in activations for ref in activation.evidence_refs if ref.get("kind") == "validation_receipt"
    )
...
def test_validate_html_report_defaults_to_lightweight_structure_mode(
    ...
    report.write_text(
        "<!doctype html><html><body><div id='chart'></div><script src='charts.js'></script></body></html>",
        encoding="utf-8",
    )
    session_manager.initialize(state)
    ...
    assert json.loads(refused_upgrade.content)["error_code"] == ("html_validation_mode_contract_mismatch")
    ...
    evaluation = _evaluate_code_validation(
        "code_validation",
        {
            "verification_activations": [
                item.model_dump(mode="json") for item in [*write_activations, *validation_activations]
            ]
        },
        {},
    )
...
def test_same_hash_success_only_supersedes_validator_invocation_failure(
    ...
    evaluation = _evaluate_code_validation(
        "code_validation",
        {"verification_activations": [item.model_dump(mode="json") for item in [*write, *failed, *passed]]},
        {},
    )
    assert evaluation.passed is expected_passed
    assert (evaluation.failure_kind.value if evaluation.failure_kind is not None else None) == expected_kind
...
def test_long_command_output_tail_classifies_missing_host_path_as_invocation_failure(
    ...
    noisy_failure = (
        ("grpc warning\n" * 300) + f"Error: Cannot find module '{target}'\n" + "code: 'MODULE_NOT_FOUND'\nExit code: 1"
    )
    ...
    receipt = next(item for item in validation_activation.evidence_refs if item.get("kind") == "validation_receipt")

    assert receipt["failure_class"] == "invocation_failure"
    assert receipt["content_observed"] is False
    execution = next(item for item in validation_activation.evidence_refs if item.get("kind") == "tool_execution")
...
def test_heatmap_contract_receipt_binds_both_exact_drafts_and_authorizes_commits(
    ...
    session_manager.transition_run_status("contract-session", run.run_id, RunStatus.RUNNING.value)
    ...
    for index, target in enumerate((html_target, js_target), start=1):
        ...
        staged[target] = session_manager.get_external_artifact_lease("contract-session", lease_id)
    ...
    assert {item["path"] for item in validated.artifact["validation_receipt"]["artifact_refs"]} == {
        str(html_target.resolve()),
        str(js_target.resolve()),
    }

    for index, (target, content) in enumerate(((html_target, html), (js_target, javascript)), start=1):
        lease = staged[target]
    ...
    assert all(set(item["related_artifact_ids"]) == registered_ids - {item["artifact_id"]} for item in registered)
...
def test_external_artifact_lease_stages_validates_and_commits_exact_target(tmp_path):
    ...
    assert (
        session_manager.get_external_artifact_lease("lease-session", lease_id)["expected_source_sha256"]
        == lease["expected_source_sha256"]
    )
...
def test_code_external_artifact_commit_requires_matching_draft_receipt(tmp_path):
    ...
    session_manager.start_harness_run("validated-lease-session", run.model_dump(mode="json"))
    session_manager.transition_run_status("validated-lease-session", run.run_id, RunStatus.RUNNING.value)
    for grant_type, capabilities in (
        ("external_file_read", ["read", "external_path"]),
        ("external_file_write", ["write", "external_path"]),
    ):
    ...
    lease_id = staged.content.split("lease_id=", 1)[1].split(";", 1)[0]
    lease = session_manager.get_external_artifact_lease("validated-lease-session", lease_id)
    assert lease is not None
    ...
    def persist_receipt(receipt: ValidationReceipt, *, call_id: str) -> None:
        activation = VerificationActivation(
            ...
            pack="code",
            status="succeeded",
            evidence_refs=[{"kind": "validation_receipt", **receipt.model_dump(mode="json"), "material": True}],
        )
        session_manager.append_run_verification_activation(
            "validated-lease-session",
            "run-1",
...
def test_artifact_failure_cannot_be_cleared_by_same_hash_success() -> None:
    ...
    assert [item["validation_receipt_id"] for item in accepted] == ["browser-later-passed"]
    assert [item["validation_receipt_id"] for item in blocking] == ["browser-artifact-failed"]
...
def test_patch_file_warns_when_mutation_stales_passing_validation_receipt(
    tmp_path,
) -> None:
    from graph.permissioned_filesystem_backend import PermissionedCompositeBackend
    from graph.session_manager import session_manager
    ...
    workspace.mkdir()
    report = workspace / "report.html"
    report.write_text("<!doctype html><title>Report</title>\n", encoding="utf-8")
    session_manager.initialize(state)
    session_manager.create_session("stale-receipt-session")
    run = RunRecord(
        run_id="run-stale-receipt",
        query_id="query-stale-receipt",
        session_id="stale-receipt-session",
        objective=f"生成 {report}",
        status=RunStatus.PREPARING,
    )
    session_manager.start_harness_run(
        "stale-receipt-session",
        run.model_dump(mode="json"),
    )
    session_manager.transition_run_status(
        "stale-receipt-session",
    ...
    session_manager.append_run_verification_activation(
        ...
        run.run_id,
        {
            "activation_id": "verification-activation-stale",
            "run_id": run.run_id,
            "query_id": run.query_id,
            ...
            "evidence_refs": [
                {
                    ...
                    "validation_receipt_id": "validation-stale-1",
                    "run_id": run.run_id,
                    "validator_kind": "html_structure",
                    "artifact_refs": [
                        {
                            "artifact_id": "artifact-stale-1",
    ...
    patch_tool = next(tool for tool in VersionedPatchMiddleware(backend).tools if tool.name == "patch_file")

    patched = patch_tool.func(
        file_path="/workspace/report.html",
        expected_sha256=content_sha256,
        replacements=[ReplacementHunk(old_string="Report", new_string="Report v2")],
        runtime=_runtime(
            "patch-stale",
            session_id="stale-receipt-session",
            run_id=run.run_id,
    ...
    assert payload["invalidated_validation_receipt_ids"] == ["validation-stale-1"]
    assert "失效" in payload["validation_invalidation_warning"]
    assert "Report v2" in report.read_text(encoding="utf-8")


def test_patch_file_without_bound_receipts_emits_no_stale_warning(tmp_path) -> None:
    ...
    patched = patch_tool.func(
        file_path="/notes.txt",
        expected_sha256=_digest("alpha\n"),
        replacements=[ReplacementHunk(old_string="alpha", new_string="beta")],
        runtime=_runtime("patch-clean"),
    )
