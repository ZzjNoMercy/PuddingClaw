...
flowchart TD
    ...
    T --> B["Docker / Restricted Host / 业务 Tool"]
...
| 类别 | 智能模式 |
|---|---|
| Docker/project/lease 内普通 Python、Node、本地计算、写文件、构建、测试、格式化 | 自动放行 |
| 公共安全 URL 的只读 fetch/search | 自动放行 |
| 安全本地 Git（add/commit/stash/switch 等，不覆盖修改） | 自动放行并记录 |
| Docker 安装依赖与临时联网 | Session 级 package/network HITL，可复用同 scope grant |
| raw network、未知域名、下载后执行 | HITL |
| 外部文件/目录写回、外部业务副作用 | HITL + receipt/plan |
| 递归删除、危险 Git、权限/所有权修改 | HITL 或拒绝 |
| sudo、Docker control、宿主越界、跨 lease、Harness 内部目录 | hard deny |
...
- CPU、内存、PID 限制可配置；
- 容器只负责隔离，命令仍要过权限管线；
- Docker 不可用时可选择降级到 Restricted Host Terminal 或拒绝执行。

Restricted Host 是 best-effort 降级，不能在 UI/Trace 宣称为真沙箱。
...
| `web_research` | `tavily_search`, `fetch_url` |
...
| Docker 项目容器、ExternalArtifact/Directory/Attachment lease | PuddingClaw | 保留精确目标身份与可审计写回 |
...
4. smart/strict 下分别 allow、ask 还是 deny？
5. 是否扩大 Backend、workspace、lease、network 或外部副作用？
6. 成功后产生什么 receipt、Artifact 或 Evidence？
7. 会激活哪个 verification pack？
8. deterministic verifier 是否真实存在？
9. Global Summary、Tool Context Compaction 是否保留其权威信息？
10. HITL 恢复时 tool-call protocol 是否完整？
11. Subagent 能否继承，能力是否只能收缩？
12. UI 如何展示运行、审批、缺口和正式完成？
